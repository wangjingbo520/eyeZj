...
public class LFilePickerActivity extends AppCompatActivity {
    ...
    private void initListener() {
        ...
        mBtnAddBook.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mTvPath.getText().toString().length() < 2) {
                    Toast.makeText(LFilePickerActivity.this, R.string.NotPath, Toast.LENGTH_SHORT).show();
                } else {
                    //返回
                    chooseBtnDone(mTvPath.getText().toString());
                }
            }
        });
    }
     ...
     * 完成提交
     */
    private void chooseBtnDone(String filesPath) {
        Intent intent = new Intent();
        intent.putExtra("paths", filesPath);
        setResult(RESULT_OK, intent);
        this.finish();
    }
